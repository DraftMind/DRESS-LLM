...
parser.add_argument('--engine', type=str, choices=['hf', 'vllm', 'mygenerate'], default='hf', help='Inference engine: hf (Transformers) or vllm')
...
if args.engine in ['hf', 'mygenerate']:
...
if args.engine == 'hf':
    ...
    top_heads_num = args.model_dir[index_a + len(a): index_b]
    alpha = args.model_dir[index_b + len(b): ]
    probes = np.load(f"{args.session_path}/features/probes_{top_heads_num}.npy")
    top_heads = np.load(f"{args.session_path}/features/top_heads_{top_heads_num}.npy")
    np.load.__defaults__=(None, False, True, 'ASCII')
    with open(f"{args.session_path}/features/activations_{top_heads_num}_{alpha}.pkl", 'rb') as f:
    ...
    print("分解完毕")
    with tqdm(total=len(prompts), desc="Generating answers", unit="sample") as pbar:
        output_data = []
        for index, question in enumerate[Any](prompts):        
            inputs = tokenizer(question, return_tensors='pt').to(model.device)
            q_tokens = inputs.input_ids
            w0 = get_activations(q_tokens.to(model.device))
            sequence = my_generate(w0, q_tokens.to(model.device), inputs)
            answer = tokenizer.decode(sequence, skip_special_tokens=False)
            answers.append(answer)
            pbar.update(1)
            output_data.append({
                "question": question,
                "daiyu_answer": answers[index],
